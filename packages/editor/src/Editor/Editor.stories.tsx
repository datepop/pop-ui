...
const EditorWithState = ({ initialValue = EMPTY_CONTENT, ...props }: TEditorWithStateProps) => {
...
export const AllBlocks: StoryObj<typeof EditorComponent> = {
  ...
  render: () => (
    <EditorWithState
      initialValue={ALL_BLOCKS_SAMPLE}
      toolbar
      inlineToolbar
      onProcessImageFiles={async (files) =>
        files.map((file) => ({ originalSrc: '', newSrc: URL.createObjectURL(file), file }))
...
const PlaygroundWrapper = ({
  ...
  const [value, setValue] = useState<TEditorElement[]>(EMPTY_CONTENT);
