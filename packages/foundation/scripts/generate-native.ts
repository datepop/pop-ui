...
const BRAND_SRC = join(ROOT, 'src/brand');
const BRAND_NATIVE_DST = join(ROOT, 'src/brand/native');

type TSourceKind = 'icon' | 'illustration' | 'brand';
...
function validateConvertedFile(content: string, fileName: string): void {
  const residualPatterns = [
    ...
    { pattern: /from ['"]\.\.\/types\/brand['"]/, label: "from '../types/brand'" },
...
function convertFile(content: string, kind: TSourceKind): string {
  ...
  if (kind === 'illustration') {
    ...
    result = result.replace(/from ['"]\.\.\/types\/brand['"]/g, "from '../../types/brand.native'");
  } else {
...
function processDirectory(srcDir: string, dstDir: string, kind: TSourceKind): number {
  ...
  for (const file of files) {
    ...
    const converted = convertFile(content, kind);
...
const iconCount = processDirectory(ICONS_SRC, ICONS_NATIVE_DST, 'icon');
console.log(`Done! ${iconCount} icons generated.\n`);

console.log('Generating React Native illustrations...');
const illustrationCount = processDirectory(
  ILLUSTRATIONS_SRC,
  ILLUSTRATIONS_NATIVE_DST,
  'illustration',
);
console.log(`Done! ${illustrationCount} illustrations generated.\n`);

console.log('Generating React Native brand icons...');
const brandCount = processDirectory(BRAND_SRC, BRAND_NATIVE_DST, 'brand');
console.log(`Done! ${brandCount} brand icons generated.`);
